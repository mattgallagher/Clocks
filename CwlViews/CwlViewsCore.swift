...
//
//  This file was generated by the CwlConcat tool on 2018-10-02 11:10:22 +0000 from the following files:
...
extension BindingName where Binding: BaseBinding {
	...
	static var cancelOnClose: BindingName<Dynamic<[Cancellable]>, Binding> { return BindingName<Dynamic<[Cancellable]>, Binding>({ v in .baseBinding(BaseBinder.Binding.cancelOnClose(v)) }) }
}
...
extension TargetAction {
	func apply<Source: TargetActionSender, Value>(instance: Source, constructTarget: () -> SignalActionTarget, processor: @escaping (Any?) -> Value) -> Cancellable? {
		switch self {
...
struct BinderAdditionalParameters<Instance, Binding, Additional>: BinderParameters {
	...
	let bindings: [Binding]
	init(subclass: Instance.Type = Instance.self, additional: Additional, bindings: [Binding] = []) {
		self.subclass = subclass
...
typealias AdhocBinderState<Instance, Binding, Additional> = BinderState<Instance, BinderAdditionalParameters<Instance, Binding, Additional>>
...
extension DerivedPreparer {
	...
	mutating func applyBindings(_ bindings: [EnclosingBinder.Binding], instance: EnclosingBinder.Instance, storage: EnclosingBinder.Storage, additional: ((EnclosingBinder.Instance) -> Cancellable?)?, combine: (EnclosingBinder.Instance, EnclosingBinder.Storage, [Cancellable]) -> Void) {
		// Prepare.
...
struct Layout {
	...
	fileprivate func twoPointConstraint<First, Second>(firstSource: NSLayoutAnchor<First>, firstTarget: NSLayoutAnchor<First>, secondSource: NSLayoutAnchor<Second>, secondTarget: NSLayoutAnchor<Second>, secondPriority: Dimension.Priority = .userHigh, secondRelationLessThan: Bool? = nil, constraints: inout [NSLayoutConstraint]) {
		...
		if let high = secondHigh {
			...
			high.priority = secondPriority
			high.isActive = true
			constraints.append(high)
		} else {
			secondLow.priority = secondPriority
		}
...
extension Layout.Dimension.Priority {
		...
		#else
			static let userLow = NSLayoutPriorityDefaultLow * 0.625
			static let userMid = NSLayoutPriorityRequired * 0.4375
		...
		#else
			static let userLow = UILayoutPriorityDefaultLow * 0.625
			static let userMid = UILayoutPriorityRequired * 0.4375
